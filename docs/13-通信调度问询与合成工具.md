...
把这五条放在一起看，这一章十把工具其实没那么散——它们都在解决同一个工程题：**怎么在不让 Agent 飞走的前提下，给它一条向外伸的窄通道**。每一条通道的窄宽度都经过校准，每一条都在缝隙里塞进了一点"如果你滥用我，我会怎么挡你"的工程兜底。读完工具描述只能学到三分之一，读完源码常量与提示词文本才能学到三分之二，最后那三分之一要靠把它们放在一起看才能学到——那也是把它们放在同一章里讲的理由。
