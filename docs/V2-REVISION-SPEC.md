...
§0.5.4 中关于「新章 frontmatter 必须含 `新增章节: yes`」的旧条款由本条覆盖：`新增章节` 这个字段也不进章节文件；如果 CI 需要识别「这是新增章节」用以触发 C-3 代码块占比闸，请改由 spec 的 `§6.2 反向矩阵 / 全新增比例` 列、或附录 manifest 中的章节清单来识别，**不**从章节文件 frontmatter 取值。
...
3. **字数与代码占比对齐 v1**：v1 已发布章节的散文：代码块字符比大致在 4:1 到 6:1 之间。新章必须落在这个区间。代码占比超 25% 触发 CI fail（C-3）。CI 识别「新章」的判据由 §6.2 反向矩阵 `全新增比例` 列或附录 manifest 章节清单提供——**不**通过章节文件 frontmatter（§0.1.1 禁止 frontmatter）。v2 实际文件采用 `NN-标题.md` 命名，文件名中不嵌入书脊编号 `C04 / C13`。
...
> **新章识别口径（YAO-99 修订）**：8 篇新增章节（C04 / C13 / C17 / C24 / C25 / C28 / C29 / C30）的「新章」身份**不再**由章节文件的 frontmatter 字段（旧条款的 `新增章节: yes`）声明——按 §0.1.1，章节文件本身禁止任何 frontmatter。CI C-3（代码块占比闸）识别新章的判据改为：由 spec §6.2 反向矩阵的 `全新增比例` 列、或附录 manifest 的章节清单提供新章列表。v2 实际文件采用 `NN-标题.md` 命名，文件名内也不嵌入书脊编号。
