...
一件是前文反复讲的 **TaskState**——`Task.ts` 里那 7 个 wire 字面量，对应"后台跑了哪些进程 / Agent / 会话"，本质是一套运行时的进程监督模型。另一件是 `utils/tasks.ts` 里的 **TodoV2**，对应"这一轮对话里我打算分几步做"，本质是一个给模型自己整理思路用的 todo 看板。

两套体系的代码几乎不交叉：字面量不同、状态机不同、持久化路径不同、UI 渲染入口也不同。但它们都对模型暴露了以 `Task` 开头的工具，刚读源码的人极容易把它们当成同一套东西。

`tools/` 下与 task 相关的目录正好 6 个，按归属一刀切成两半：

| 工具目录 | 服务的体系 | 入口源码 | 启用门 |
|---|---|---|---|
| `tools/TaskStopTool/` | TaskState | `tools/TaskStopTool/TaskStopTool.ts:39-131` | 默认启用（别名 `KillShell`） |
| `tools/TaskOutputTool/` | TaskState | `tools/TaskOutputTool/TaskOutputTool.tsx:144-308` | 默认启用（别名 `AgentOutputTool` / `BashOutputTool`），已标记 deprecated |
...
**TaskStopTool** 的 `call()` 是一层薄壳，真正的逻辑全部委派给 `tasks/stopTask.ts:38-100` 的 `stopTask()`：

```typescript
// tools/TaskStopTool/TaskStopTool.ts:107-130
async call({ task_id, shell_id }, { getAppState, setAppState }) {
  const id = task_id ?? shell_id        // KillShell 旧名兼容
...
`stopTask()` 内部依次做三件事：按 `task.type` 通过 `getTaskByType()` 找到对应的 `Task` 实现、调用它的 `kill()`、再决定要不要发通知。最后一步分两条路：

- **`local_bash` 走"通知抑制"分支**：bash 被 kill 时正常会走通知队列，但 exit code 137 本身没有信息量，源码在这里把 `notified` 直接翻成 true 跳过 XML 通知，转而通过 `emitTaskTerminatedSdk()` 给 SDK 消费者发一条精简事件。
- **Agent 任务不抑制**：因为 AbortError 的 catch 分支会发一条通知，里头携带 `extractPartialResult(agentMessages)`——这是真正有信息量的"半成品"，不能丢。
...
**TaskOutputTool** 则是另一种姿态——它从一开始就被标记为 deprecated。两处声明把这个倾向说得很清楚：

- `tools/TaskOutputTool/TaskOutputTool.tsx:157-159` 的 `description()` 返回 `'[Deprecated] — prefer Read on the task output file path'`。
- `tools/TaskOutputTool/TaskOutputTool.tsx:172-181` 的 `prompt()` 首行写着 `DEPRECATED: Prefer using the Read tool on the task's output file path instead.`

也就是说，源码作者建议模型优先用通用的 `Read` 工具去读 `TaskState.outputFile` 这条路径，不要再调一个专门的工具拉日志。但 TaskOutputTool 又必须保留——因为 `AgentOutputTool` / `BashOutputTool` 这两个旧别名还活在历史 transcripts 里，一旦移除，老 session resume 时会报"找不到工具"。
...
**状态机也不一样**：TodoV2 用的字面量是 `pending` / `in_progress` / `completed`，更新时还可以设为 `deleted`；TaskState 用的是 `pending|running|completed|failed|killed`。两套字面量没有交集，对模型暴露的语义也不同——前者是"todo 项目的进度"，后者是"后台进程的生命周期"。

四个工具的职责对称地切成 CRUD：

- **TaskCreateTool**（`tools/TaskCreateTool/TaskCreateTool.ts:80-129`）创建一条 todo，同时跑 `executeTaskCreatedHooks()`；hook 抛出 blocking error 时，刚创建的 task 会被 `deleteTask()` 回滚。创建后还会把 UI 的 `expandedView` 切到 `tasks`，强制把面板展开给用户看。
- **TaskListTool**（`tools/TaskListTool/TaskListTool.ts:65-90`）只读拉清单，会过滤掉 `metadata._internal` 的内部 task，并把每条 task 的 `blockedBy` 里已 completed 的 ID 剔掉——避免"被一个已完成的任务阻塞"这种噪音。
- **TaskGetTool**（`tools/TaskGetTool/TaskGetTool.ts:73-97`）按 id 取单条，返回完整的 `description` / `blocks` / `blockedBy`。
- **TaskUpdateTool**（`tools/TaskUpdateTool/TaskUpdateTool.ts:123-363`）做更新，是这套体系里最重的一个：除了改字段，还要在 `status = completed` 时跑 `executeTaskCompletedHooks()`；在 swarm 模式下，把刚被切为 `in_progress` 的 task 自动归属当前 agent；如果 owner 字段发生变化，向新 owner 的 mailbox（`utils/teammateMailbox.ts`）写一条 `task_assignment` 消息，让被指派的 teammate 在自己的下一轮里看到。

`TaskUpdateTool` 的尾巴上还埋了一个**验证提醒**：
...
[第 17 章：Coordinator、Cron 与定时调度 — 让会话在没人按回车时继续转](./17-Coordinator-Cron-与定时调度.md)

我们换一个角度：如果根本没人盯着这个 REPL，事情还能继续往前推吗？看 coordinator/、ScheduleCronTool 与 useScheduledTasks 如何让会话在没人按回车时继续转。
